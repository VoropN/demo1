...
export const test6 = () => describe('Numerical Sequence', function () {
  ...
  describe('Should return error', function () {
    it('if length or minSqrt is not defined! => numericalSequence({ length: "", minSqrt: 100 })', function () {
      chai.assert.deepEqual(numericalSequence({ length: "", minSqrt: 100 }), { reason: ['Length is not defined!'] });
    });
    it('if length or minSqrt is missing! => numericalSequence({ minSqrt: 100 })', function () {
      chai.assert.deepEqual(numericalSequence({ minSqrt: 100 }), { reason: ['Length is not defined!'] });
    });
    it('if an empty object is passed! => numericalSequence({})', function () {
      chai.assert.deepEqual(numericalSequence({}), { reason: ['Length is not defined!', 'MinSqrt is not defined!'] });
    });
    it('if arguments not passed! => numericalSequence()', function () {
      chai.assert.deepEqual(numericalSequence(), { reason: ['Arguments not passed!'] });
    });
    it('if one of arguments is not correct! => numericalSequence({ length: 10, minSqrt: "d" })', function () {
    ...
    it('if one of arguments is negative value! => numericalSequence({ length: -10, minSqrt: 100 })', function () {
      chai.assert.deepEqual(numericalSequence({ length: -10, minSqrt: 100 }), { reason: ['Length can not be less than zero!'] });
    });
    ...
    it('if one of arguments is not integer! => numericalSequence({ length: 1.1, minSqrt: 1000 })', function () {
