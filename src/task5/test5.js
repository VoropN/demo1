...
export const test5 = () => describe('Happy Tickets', function () {
  ...
  describe('Should return error', function () {
    it('if empty object is passed! => new HappyTickets({}).result', function () {
      chai.assert.deepEqual(new HappyTickets({}).result, { reason: ['Min is not defined!', 'Max is not defined!'] });
    });
    ...
    it('if max is less than 10! => new HappyTickets({ min: 1, max: 9 }).result', function () {
      chai.assert.deepEqual(new HappyTickets({ min: 1, max: 9 }).result, { reason: ['Max can not be less than 10!'] });
    });
    ...
    it('if max or min value is not valid format! => new HappyTickets({ min: "f", max: 9998 }).result', function () {
    ...
    it('if max or min value is negative value! => new HappyTickets({ min: -25, max: 9998 }).result', function () {
      chai.assert.deepEqual(new HappyTickets({ min: -25, max: 9998 }).result, { reason: ['Min can not be less than zero!'] });
    });
